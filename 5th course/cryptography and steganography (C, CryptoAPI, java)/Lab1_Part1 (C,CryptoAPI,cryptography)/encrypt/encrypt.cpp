...
int _tmain(int argc, _TCHAR* argv[]) {
	...
	if (_tcscmp(argv[3], TEXT("CALG_RC2")) == 0) {
		...
		ENCRYPT_BLOCK_SIZE = 128;
		KEYLENGTH = 0x00C00000; // 192 bit
		PROVIDER = MS_ENH_RSA_AES_PROV;
		PROVIDER_TYPE = PROV_RSA_AES;
	} else {
		ENCRYPT_ALGORITHM = CALG_AES_256;
		ENCRYPT_BLOCK_SIZE = 128;
		KEYLENGTH = 0x01000000; // 256 bit
		PROVIDER = MS_ENH_RSA_AES_PROV;
		PROVIDER_TYPE = PROV_RSA_AES;
	}
...
bool MyEncryptFile(
	...
	if (CryptAcquireContext(&hCryptProv, NULL, PROVIDER,
		PROVIDER_TYPE, 0) == NTE_BAD_KEYSET) {
		if (CryptAcquireContext(&hCryptProv, NULL, PROVIDER,
