	...
	if ($requestType === INPUT_POST || $requestType === INPUT_GET) {
		...
		if ($valid) {
			try {
				...
				$sql = "INSERT INTO employees (employeeLastName, homeAddress, age, phone, chairId)
						VALUES (?, ?, ?, ?, (SELECT chairs.id FROM chairs 
							WHERE chairs.chairName = ?));";
				$q = $pdo->prepare($sql);
				$q->execute(array(
...
<head>
	...
	<![endif]-->
	<title>Работа №3, Добавление сотрудника</title>
