...
<div class="masthead">
    <div class="site-center">
        <div class="cell">
            <h4>Работа 3, База данных, Автомобили</h4>
        </div>
    </div>
</div>
<?php
...
<?php
	try {
		...
		$sql = 'SELECT model, color, manufactureDate, price, percentDeduction, 
				owners.fullName AS owner, buyers.fullName AS buyer 
				FROM cars, owners, buyers
				WHERE cars.idOwner = owners.id AND cars.idBuyer = buyers.id;';
		$res = $pdo->query($sql);
		if ($res->rowCount() == 0) { 
