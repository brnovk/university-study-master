<?php
	...
	try {
		...
		$sql = 'SELECT fullName FROM owners;';
		$ownerList = $pdo->query($sql);
		if ($ownerList->rowCount() == 0) {
			$errors .= 'В таблице продавцов нет ни одной записи.<br>';
			$ownerList = null;
		} else {
			$ownerList = $ownerList->fetchAll();
		}
		Database::disconnect();
	...
	try {
		$pdo = Database::connect();
		$sql = 'SELECT fullName FROM buyers;';
		$buyerList = $pdo->query($sql);
		if ($buyerList->rowCount() == 0) {
			$errors .= 'В таблице покупателей нет ни одной записи.<br>';
			$buyerList = null;
		} else {
			$buyerList = $buyerList->fetchAll();
		}
		Database::disconnect();
	} catch(PDOException $e) {
		$errors .= 'Произошла ошибка при работе с БД.<br>'.$e->getMessage();
	...
	if ($requestType === INPUT_POST || $requestType === INPUT_GET) {
		...
		if ($valid) {
			try {
				...
				$sql = "INSERT INTO cars (model, color, manufactureDate, price, 
						percentDeduction, idOwner, idBuyer) VALUES (?, ?, ?, ?, ?, 
  						(SELECT owners.id FROM owners WHERE owners.fullName = ?),
  						(SELECT buyers.id FROM buyers WHERE buyers.fullName = ?));";
				$q = $pdo->prepare($sql);
				$q->execute(array(
					$model,
					$color,
...
<div class="masthead">
    <div class="site-center">
        <div class="cell">
            <h4>Работа 3, Добавление автомобиля</h4>
        </div>
    </div>
</div>
<?php if (!empty($errors)): ?>
