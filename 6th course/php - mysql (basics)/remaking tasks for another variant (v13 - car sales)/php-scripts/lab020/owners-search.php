<?php
	...
	if ($requestType === INPUT_POST || $requestType === INPUT_GET) {
		...
		if ($valid) {
			$searchError = null;
			$searchResult = searchOwners($searchField, $searchValue, $searchError);
		...
		$errors .= 'Ошибка загрузки параметров поиска. 
			Повторите загрузку результатов со страницы "Владельцев"';
	}
...
<div class="masthead">
    <div class="site-center">
        <div class="cell">
            <h4>Работа 2, Поиск владельцев</h4>
        </div>
    </div>
</div>
<?php if (!empty($errors)): ?>
