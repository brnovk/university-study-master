<?php
	...
	if (empty($employeeList) || count($employeeList) == 0) { 
		$errors .= 'В таблице сотрудников нет ни одной записи.<br>';
		$employeeList = null;
	} else {
		$bufferList = null;
		$tmpWages = getWages();
		foreach ($employeeList as $currentEmployee) {
			$found = false;
			foreach ($tmpWages as $currentWage) {
				if (strcmp(mb_strtolower($currentEmployee['employeeLastName'],'UTF-8'), 
							mb_strtolower($currentWage['employeeLastName'],'UTF-8')) == 0) {
					$found = true;
					break;
				}
			}
			if (!$found) {
				$bufferList[] = $currentEmployee;
			}
		}
		$employeeList = $bufferList;
	...
	if ($requestType === INPUT_POST || $requestType === INPUT_GET) {
		...
		if ($valid) {
			$res = addWage($appendSalary,$appendCredited,$appendWithheld,$appendEmployeeLastName);
			if (!is_null($res)) {
				$errors .= 'Ошибка добавления зарплаты: '.$res.'<br>';
			} else {
...
<head>
	<meta charset="utf-8">
	<title>Лабораторная №2, Добавление зарплаты</title>
