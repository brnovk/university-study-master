...
namespace ConnectNNet.Server
{
	...
	public class WcfServer : IServer
	{
		...
		public Guid Start(GameSession message)
		{
			CleanSessions();
			foreach (var game in Games.Values)
			{
				...
				{
					game.GameInstance.Enemy = message.GameInstance.Player;
					game.LastPlayerBConnection = DateTime.Now;
					game.State = GameState.Started;
					return game.CreatorId;
				}
			}
			message.GameInstance.InProcess = false;
			Games[message.CreatorId] = message;
			message.State = GameState.Starting;
		...
		private void NextState(GameSession game)
		{
			switch (game.State)
			{
